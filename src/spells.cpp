...
const std::vector<Spell> kSpellTable = {
    ...
    // pools (Imp/Demon/Raise Dead) below. Each pool is its own cap, not a slice of a
    // shared total — see the note on Spell::minion_cap for why there's deliberately no
    // combined ceiling across summon sources.
